  ...
  namespace :jungle do
    ...
    task :install do
      on roles(fetch(:puma_role)) do |role|
        ...
        sudo_if_needed "mv #{fetch(:tmp_dir)}/run-puma #{fetch(:puma_run_path)}"
        ...
        sudo_if_needed "touch #{fetch(:puma_jungle_conf)}"
      end
    end
    ...
    def debian_install
      ...
      sudo_if_needed "mv #{fetch(:tmp_dir)}/puma /etc/init.d/puma"
      sudo_if_needed 'update-rc.d -f puma defaults'
    ...
    def rhel_install
      ...
      execute "chmod +x #{fetch(:tmp_dir)}/puma"
      sudo_if_needed "mv #{fetch(:tmp_dir)}/puma /etc/init.d/puma"
      sudo_if_needed 'chkconfig --add puma'
    end
    ...
    task :add do
      on roles(fetch(:puma_role)) do|role|
        sudo_if_needed "/etc/init.d/puma add '#{current_path}' #{fetch(:puma_user, role.user)}"
      end
    end
    ...
    task :remove do
      on roles(fetch(:puma_role)) do
        sudo_if_needed "/etc/init.d/puma remove '#{current_path}'"
      end
    end
    ...
    %w[start stop restart status].each do |command|
      ...
      task command do
        on roles(fetch(:puma_role)) do
          sudo_if_needed "service puma #{command} #{current_path}"
        end
      end
    end
